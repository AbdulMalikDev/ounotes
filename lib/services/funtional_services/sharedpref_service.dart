...
class SharedPreferencesService{
  ...
  Future<bool> isUserLoggedIn() async
  {
    ...
    SharedPreferences prefs = await store();
    if(!prefs.containsKey("current_user")){return false;}
    else{
  ...
  storeReport(List<Report> reports) async{
    ...
    prefs.setString("reports", json.encode(reports.map((e) => e.toJson()).toList()));
  }

  loadReportsFromStorage() async {
     SharedPreferences prefs = await store();
    if(!prefs.containsKey("reports")){return null;}
    else{
      List reportsLocal = json.decode(prefs.getString("reports"));
      List<Report> reports = reportsLocal.map((e) => Report.fromData(e)).toList();
      return reports;
    }
  }
  ...
  {
    if(introDialog!=null){if(introDialog==false){return introDialog;}}
    SharedPreferences prefs = await store();
